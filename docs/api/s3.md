...
optionally call `.with_namespace(prefix)`, and inject it with
...
Unlike Redis, `S3Client` does not retain a live connection. Creating a cheap
client wrapper for each fixed namespace keeps S3 path resolution simple and
avoids adding a resolver/context lookup to every S3 operation.
